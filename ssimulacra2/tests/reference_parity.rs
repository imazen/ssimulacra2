...
// Reference data will be generated by capture_cpp_reference example
// For now, we'll create a stub module
mod reference_data {
    #[derive(Debug, Clone)]
    ...
    // This will be replaced by auto-generated data
    pub const REFERENCE_CASES: &[ReferenceCase] = &[];
}

use reference_data::{ReferenceCase, REFERENCE_CASES};
...
fn generate_test_image(case: &ReferenceCase) -> (Vec<u8>, Vec<u8>) {
    ...
    } else if name.starts_with("gradient_h") {
        let grad = gen_gradient_h(width, height);
        (grad.clone(), grad)
    } else if name.starts_with("gradient_v") {
        let grad = gen_gradient_v(width, height);
        (grad.clone(), grad)
    } else if name.starts_with("gradient_diag") {
