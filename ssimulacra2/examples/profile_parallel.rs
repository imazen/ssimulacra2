...
fn main() {
    ...
    for _ in 0..6 {
        ...
        let _img1_planar: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
        let _img2_planar: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
        let _sigma1_sq: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
        let _sigma2_sq: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
    ...
    println!("   Sequential (6 scales): {:.2}ms", sequential_alloc.as_secs_f64() * 1000.0);

    // Parallel allocation
    let alloc_start = Instant::now();
    let _results: Vec<_> = (0..6).into_par_iter().map(|_| {
        let _mul: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
        let _img1_planar: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
        let _img2_planar: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
        let _sigma1_sq: [Vec<f32>; 3] = [vec![0.0f32; size], vec![0.0f32; size], vec![0.0f32; size]];
    ...
    println!("   Parallel (6 scales):   {:.2}ms", parallel_alloc.as_secs_f64() * 1000.0);
    println!("   Allocation slowdown:   {:.2}x\n", parallel_alloc.as_secs_f64() / sequential_alloc.as_secs_f64());

    // Measure buffer sizes
    let total_per_scale = size * 4 * 8 * 3; // 8 arrays of 3 planes, f32 each
    println!("2. Memory usage per scale:");
    println!("   Scale 0 (2048x2048): {} MB per task", total_per_scale / 1024 / 1024);
    println!("   Total if all 6 run parallel: {} MB\n", total_per_scale * 6 / 1024 / 1024);

    // Full sequential run
    println!("3. Full computation comparison:");
    let warmup_src = Rgb::new(source.data().to_vec(), width, height, TransferCharacteristic::SRGB, ColorPrimaries::BT709).unwrap();
    let warmup_dst = Rgb::new(distorted.data().to_vec(), width, height, TransferCharacteristic::SRGB, ColorPrimaries::BT709).unwrap();
    ...
    for _ in 0..iterations {
        let src = Rgb::new(source.data().to_vec(), width, height, TransferCharacteristic::SRGB, ColorPrimaries::BT709).unwrap();
        let dst = Rgb::new(distorted.data().to_vec(), width, height, TransferCharacteristic::SRGB, ColorPrimaries::BT709).unwrap();
    ...
    println!("\n4. Overhead analysis:");
    println!("   Ideal parallel time ({}x):  {:.2}ms", num_cpus, ideal_parallel);
    println!("   Actual parallel time:        {:.2}ms", par_time);
    println!("   Overhead:                    {:.2}ms ({:.0}%)", overhead, overhead / seq_time * 100.0);
}
