...
fn generate_test_cases() -> Vec<TestCase> {
    ...
    for (width, height) in sizes {
        ...
        for shift in [1, 5, 10, 20, 50] {
            ...
            let distorted = TestImageGenerator::uniform(
                width,
                height,
                128 + shift,
                128 + shift,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, case) in test_cases.iter().enumerate() {
        ...
        save_png(&distorted_path, &case.distorted_data, case.width, case.height)?;
    ...
    println!("\nDone! Generated {} reference cases", reference_cases.len());
...
fn generate_reference_file(
    ...
    writeln!(f, "//!")?;
    writeln!(f, "//! Generated by: cargo run --example capture_cpp_reference")?;
    writeln!(
        f,
        "//! Date: {}",
    ...
    writeln!(f, "//!")?;
    writeln!(f, "//! This file contains reference values captured from the C++ ssimulacra2")?;
    writeln!(
        f,
    ...
    writeln!(f)?;
    writeln!(f, "/// A reference test case with expected C++ ssimulacra2 score.")?;
